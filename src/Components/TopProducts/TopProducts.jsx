...
export default function TopProducts({ onViewMore }) {
  ...
  const handleClick = () => {
    ...
    navigate("/Catalog"); // replace with your desired route
  };
  ...
  return (
    <div className="glow-wrapper py-12 md:py-20 px-4">
      {/* Blurred Glow Elements */}
      <div className="glow glow-pink"></div>
      <div className="glow glow-purple"></div>

      {/* All content */}
      <div className="glow-content max-w-5xl mx-auto" ref={sectionRef}>
        {/* Header */}
        <div className="text-center mb-10 md:mb-16">
          <h2
            ...
            style={{
              background:
                "linear-gradient(135deg, #1c4174 0%, #1a437c 50%, #ff7b16 100%)",
        ...
        >
          <button
            className="px-8 py-3 bg-gradient-to-r from-[#1c4c97] to-[#0a0e27] hover:from-[#ff7b16] hover:to-[#1c4c97] text-white font-bold shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
            onClick={handleClick}
          >
            View All Categories
          </button>
