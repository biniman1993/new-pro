...
const Partners = () => {
  ...
  return (
    <section ref={sectionRef} className="py-12 md:py-20 bg-[#f8fafc] w-full overflow-hidden font-sans">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="text-center mb-12 md:mb-20">
          ...
          <p className="text-gray-500 max-w-2xl mx-auto text-sm md:text-lg">
            Empowering Ethiopia's leading institutions with world-class technology solutions.
          </p>
        </div>
            ...
            >
              ...
              <div className="relative aspect-[4/3] flex items-center justify-center p-4 md:p-8 bg-gray-50/50 group-hover:bg-white transition-colors duration-500">
                <img
                  src={partner.logo}
                  alt={partner.name}
                  className="max-w-full max-h-full object-contain grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all duration-700"
                />
                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  {[...Array(5)].map((_, i) => (
                    <div key={i} className="w-1.5 h-1.5 rounded-full bg-[#e67817]"></div>
                  ))}
                </div>
              </div>
              ...
              <div className="p-4 md:p-6 flex flex-col flex-grow text-center md:text-left">
                <h3 className="text-sm md:text-lg font-bold text-[#1e293b] mb-1 line-clamp-1 group-hover:text-[#2a5da5] transition-colors">
                  {partner.name}
                </h3>
                ...
                <div className="mb-4 flex-grow">
                  <p className="text-[11px] md:text-sm text-gray-500 leading-relaxed line-clamp-2 md:line-clamp-3 italic">
                    "Trusted partner in digital transformation and infrastructure."
                  </p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-2 mt-auto pt-4 border-t border-gray-50">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 px-3 bg-[#e67817] hover:bg-[#d56d15] text-white text-[10px] md:text-xs font-bold rounded-lg transition-all active:scale-95"
                  >
                    READ MORE <ArrowRight size={14} />
                  </button>
                  <button
                    className="flex items-center justify-center p-2 border border-[#2a5da5] text-[#2a5da5] hover:bg-[#2a5da5] hover:text-white rounded-lg transition-all active:scale-95"
                    title="Contact Us"
                  >
                    <Mail size={16} />
                  </button>
      ...
      {/* --- Keep your existing Modal Logic here (it is untouched) --- */}
      {selectedPartner && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[1000] p-4" onClick={closeModal}>
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto relative shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <button className="absolute top-4 right-4 w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center hover:bg-[#e67817] hover:text-white transition-all" onClick={closeModal}>
              ✕
            </button>
            <div className="p-8 md:p-12 text-center border-b border-gray-100 bg-gray-50/50">
              <img src={selectedPartner.logo} alt={selectedPartner.name} className="max-h-20 mx-auto mb-6 object-contain" />
              <h2 className="text-2xl font-bold text-gray-900 leading-snug">{selectedPartner.modalTitle}</h2>
            </div>
            <div className="p-8 md:p-12">
              <p className="text-lg leading-relaxed text-gray-700">{selectedPartner.modalBody}</p>
            </div>
          </div>
        </div>
      )}
