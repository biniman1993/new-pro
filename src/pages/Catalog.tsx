  ...
  const filteredProducts = useMemo(() => {
    return categoryProducts.filter((product) => {
      ...
      for (const [filterType, selectedValues] of Object.entries(filters)) {
        ...
        switch (filterType) {
          case "availability":
            if (selectedValues.includes("In stock") && product.count > 0)
              productMatches = true;
            if (selectedValues.includes("Out of stock") && product.count === 0)
              productMatches = true;
            // For backorder, we'll assume some products are marked as backorder
            if (selectedValues.includes("On backorder") && product.count < 5)
              productMatches = true;
            break;
      ...
      <header className="relative overflow-hidden mt-20 ">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 py-16 md:py-24 text-center">
          ...
          <p className="text-blue-100 text-lg md:text-xl max-w-2xl mx-auto font-light">
            ...
            {getCategoryDisplayName(activeCategory).toLowerCase()} products
          </p>
        </div>
      </header>
